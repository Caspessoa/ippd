...
void iniciar_log(const char* nome_arquivo) {
    // Abre no modo "w" (write) para apagar o log da execução anterior e começar limpo
    FILE* f = fopen(nome_arquivo, "w"); 
    if (f != NULL) {
        fprintf(f, "=== HISTÓRICO DA SIMULAÇÃO (MPI + OpenMP) ===\n");
        fprintf(f, "Ciclo | Estação | População Total | Energia Total | Recurso Total\n");
        fprintf(f, "---------------------------------------------------------------\n");
        fclose(f);
    } else {
